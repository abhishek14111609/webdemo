                            ...
                            <div class="col-md-8">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('slug') is-invalid @enderror"
                                           id="slug" name="slug" value="{{ old('slug', $collection->slug) }}" placeholder="Leave empty to auto-generate">
                                    @error('slug')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="mb-3">
                                    ...
                                    @if($collection->image)
                                        <div class="form-text">
                                            Current image: <a href="{{ asset('storage/' . $collection->image) }}" target="_blank">View</a>
                                        </div>
                                    @endif
                                </div>
                                        ...
                                        <div class="mb-3">
                                            <div class="form-check">
                                                <input type="checkbox" class="form-check-input @error('is_featured') is-invalid @enderror"
                                                       id="is_featured" name="is_featured" value="1" {{ old('is_featured', $collection->is_featured) ? 'checked' : '' }}>
                                        ...
                                        <div class="mb-3">
                                            <div class="form-check">
                                                <input type="checkbox" class="form-check-input @error('is_active') is-invalid @enderror"
                                                       id="is_active" name="is_active" value="1" {{ old('is_active', $collection->is_active) ? 'checked' : '' }}>
                                ...
                                <div class="card bg-light">
                                    <div class="card-body">
                                        ...
                                        <div class="text-center">
                                            <div id="image-preview" class="mb-3" style="{{ $collection->image ? 'display: block;' : 'display: none;' }}">
                                                <img id="preview-img" src="{{ $collection->image ? asset('storage/' . $collection->image) : '' }}"
                                                     alt="Preview" class="img-fluid rounded" style="max-height: 200px;">
                                            </div>
                                            <div id="no-image" class="mb-3" style="{{ $collection->image ? 'display: none;' : 'display: block;' }}">
...
document.getElementById('image').addEventListener('change', function(e) {
    ...
    if (file) {
        ...
        @if($collection->image)
            preview.src = '{{ asset('storage/' . $collection->image) }}';
            imagePreview.style.display = 'block';
            noImage.style.display = 'none';
        @else
